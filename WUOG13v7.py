...
def scrape_playlist(url, playlist_csv_filename='playlist_urls.csv', song_csv_filename='song_data.csv'):
    ...
    for playlist_item in playlist_items:
        ...
        # Check if the playlist URL is already in the CSV file and the playlist time is within the specified range
        if not is_playlist_url_duplicate(playlist_url, playlist_csv_filename) and is_valid_playlist_time(playlist_time):
            # If it's not a duplicate and meets the time criteria, append the playlist data to the CSV file for playlist URLs
...
def is_valid_playlist_time(playlist_time):
    # Check if the playlist time is between 1:00 AM and 5:00 AM
    try:
